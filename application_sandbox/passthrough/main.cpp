...
struct PassthroughFrameData {
  ...
  containers::unique_ptr<VulkanApplication::Buffer> vertex_buff;
  containers::unique_ptr<VulkanApplication::Buffer> uv_buff;
};
 ...
 public:
  ...
  virtual void InitializeFrameData(
    ...
    const auto uv_buf_create_info = GetBufferCreateInfo(
        sizeof(kUV),
        VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
    frame_data->vertex_buff =
        app()->CreateAndBindHostBuffer(&vertices_buf_create_info);
    frame_data->uv_buff = app()->CreateAndBindHostBuffer(&uv_buf_create_info);
    const ::VkBuffer vertex_buffers[2] = {*frame_data->vertex_buff, *frame_data->uv_buff};
    const ::VkDeviceSize vertex_buffer_offsets[2] = {0, 0};

    app()->FillHostVisibleBuffer(&*frame_data->vertex_buff,
                                 reinterpret_cast<const char*>(kVertices),
                                 sizeof(kVertices), 0, initialization_buffer,
                                 VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
                                 VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT);
    app()->FillHostVisibleBuffer(&*frame_data->uv_buff, reinterpret_cast<const char*>(kUV),
